...
function del(bespeakId){
  layer.confirm('删除预约?', {icon: 3, title:'提示'}, function(index){
   $.ajax({
                type:'post',
                url:'deleteBespeak',
                dataType:"json",
...
<c:if test="${!empty requestScope.BespeakAllMsg}">
						...
						<c:forEach items="${BespeakAllMsg.lists }" var="b">
							<tr>
								...
								<td>
                                   ...
                                   <c:if test="${b.bespeakState==1}">
                                   <button class="layui-btn" onclick="add(${b.bespeakId },'${b.bespeakAtten }',${b.user_Id})" >建单</button>
                                   </c:if>
                                   <button class="layui-btn " onclick="info(${b.bespeakId })" >详情</button>
                                   <button class="layui-btn" onclick="del(${b.bespeakId })">删除</button>
                                    </div>
									</div>
	  ...
	  <table class="layui-table"   border="0" cellspacing="0" cellpadding="0"  width="900px">
       ...
       <c:if test="${requestScope.BespeakAllMsg.currPage != 1}">
           <a href="${pageContext.request.contextPath }/showbespeakBefore?currentPage=1">[首页]</a>&nbsp;&nbsp;
           <a href="${pageContext.request.contextPath }/showbespeakBefore?currentPage=${requestScope.BespeakAllMsg.currPage-1}">[上一页]</a>&nbsp;&nbsp;
       </c:if>

         <c:if test="${requestScope.BespeakAllMsg.currPage != requestScope.BespeakAllMsg.totalPage&&requestScope.BespeakAllMsg.totalPage!=0}">
           <a href="${pageContext.request.contextPath }/sshowbespeakBefore?currentPage=${requestScope.BespeakAllMsg.currPage+1}">[下一页]</a>&nbsp;&nbsp;
           <a href="${pageContext.request.contextPath }/showbespeakBefore?currentPage=${requestScope.BespeakAllMsg.totalPage}">[尾页]</a>&nbsp;&nbsp;
       </c:if>
   </span>
        </td>
    </tr>
